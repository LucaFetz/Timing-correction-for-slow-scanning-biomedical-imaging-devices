...
if spline_order == 1
    ...
    h2 = @(x,y,t,k,l,m) (cubic_B_spline_scalar(x-k).*cubic_B_spline_scalar(y-l).*cubic_B_spline_scalar(t-m));%takes scalar input and gives scalar output
end
...
Hopt5 = sparse(size(Hopt4,1),size(Hopt4,2));

% for l = 1:Nx*Ny*Nt
%     Hopt3(:,l) = Hopt2(:,samples_coordinates_x(l)*Ny+samples_coordinates_y(l)+1+samples_coordinates_f(l)*Ny*Nx);
% end
l = 1:Nx*Ny*Nt;
l2 = samples_coordinates_x(l)*Ny+samples_coordinates_y(l)+1+samples_coordinates_f(l)*Ny*Nx;
Hopt5(:,l) = Hopt4(:,l2);
